...
truth_target <- 'death'
analysis_name <- 'death'
models <- c('BPagano-RtDriven', 'COVIDhub-baseline',  
            'CU-select', 'GT-DeepCOVID', 
            'Karlen-pypm', 'MOBS-GLEAM_COVID', 
            'PSI-DRAFT', 'RobertWalraven-ESG', 
            'UCSD_NEU-DeepGLEAM', 'USC-SI_kJalpha', 
            'COVIDhub-4_week_ensemble')

forecast_dates <- as.character(seq.Date(from = ymd('2020-11-02'), 
                                        to = ymd('2022-11-07'), by = 7))
forecast_target_names <- c(paste(1:4, "wk ahead inc death"))
