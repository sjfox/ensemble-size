...
get_train_test_scores <- function(file_path, 
                                  target_bounds,
                                  train_seasons = c('2010/2011', '2011/2012', '2012/2013', '2013/2014')){
  get_clean_forecast_score(file_path, target_bounds) %>% 
    mutate(period = ifelse(season %in% train_seasons, 'Train', 'Test')) |> 
    group_by(model, period) |> 
    summarize(score = exp(mean(score)), .groups = 'drop')
}
...
if(!file.exists('processed-data/multiyear-traintest_performance.rda') |
   refresh){
  ...
  list.files('processed-data/multiyear-analysis/', full.names = T) %>% 
    map(get_train_test_scores, 
        target_bounds = target_bounds) %>% 
    bind_rows(.id = 'id') -> my_traintest_scores
  save(my_traintest_scores, file = 'processed-data/multiyear-traintest_performance.rda')
} else{
  load('processed-data/multiyear-traintest_performance.rda')
}

my_traintest_scores |> 
  filter(model == 'ensem')
list.files('processed-data/multiyear-analysis/', full.names = T)[3001]
read_csv('processed-data/multiyear-analysis//ensemble-12699_scores.csv', col_types = 'ccnncc', show_col_types = FALSE) %>% 
  inner_join(target_bounds, by = c('season', 'location', 'target')) %>% 
  filter(ifelse(start_week>=38, 
                (forecast_week>=start_week | forecast_week <= end_week),
                (forecast_week>=start_week & forecast_week <= end_week)
...
my_lookup |> filter(combination_num == '12699')

my_traintest_scores |> 
  filter(period == 'Test') |> 
  mutate(combination_num = as.numeric(str_replace(model, pattern = 'ensemble-', replacement = ''))) %>% 
  left_join(my_lookup %>% distinct(combination_num, k)) |> 
  ggplot(aes(k, 1/score)) +
    geom_point(alpha = .05)
